...
- Thank-you pack (25.09.2026): a tip unlocks other tints and designs (`AppSkin`) and alternate app icons. A color swaps only the tint and the two side colors of the signature gradient. A design may additionally set the system font design (rounded/serif/monospaced), a paper tone behind the main screens (`.paper()`: recordings, areas, search, note – sheets stay system) and force Dark Mode (Terminal). Never custom controls, layouts or fonts outside the system families. Views read `.tint`; where a `Color` is needed, `@Environment(\.skin)`, never `Color.accentColor`.
