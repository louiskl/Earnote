...
| **B3 iPhone** | ✅ Code: `EarnoteiOS/App/HandoffSender.swift` (AAC über `AVAssetExportSession`, Übergabe, Rücknahme mit „Auf dem iPhone verarbeiten“, Mitteilung nach 24 h oder bei `failed`), Einstellungen und Onboarding › „Mac“ (iCloud-Schalter, „Notizen schreibt mein Mac“, sobald ein Mac gefunden ist), Status in Liste und Notiz, iCloud- und Push-Berechtigung. `ProcessingQueue.resumes` verhindert, dass iPhone und Mac dieselbe Aufnahme verarbeiten. Nach der fertigen Notiz vom Mac löscht das iPhone sein eigenes Audio, wenn „Audio behalten“ aus ist (`HandoffSender.refresh`). **Offen:** am Gerät testen |
| **B4 Geräte** | ✅ 24.09.2026, iCloud **Development** (iPhone 15 + MacBook Air M1, Mac in `EARNOTE_SANDBOX`): iPhone findet den Mac, Übergabe → Mac übernimmt, transkribiert, Notiz (lokale KI) → zurück aufs iPhone, Übergabe in iCloud gelöscht. Dabei behoben: Mac ohne Push (holte nur beim Start ab) → `aps-environment` + `registerForRemoteNotifications`; fest 60 s Warten nach dem Beanspruchen → nur noch bei zweitem Mac (`HandoffRules.mustSettleClaim`); Neustart setzte fremde, über iCloud gekommene Aufnahmen fort → nur mit Audio auf dem Gerät; iPhone 15 bekam Apple Intelligence als Standard → `NoteWay.worksHere`. Dauer danach: ca. 1–1,5 min vom Stopp bis zur Notiz (davon ~50 s iCloud). Erster Lauf einer neu gebauten Mac-App: Whisper wird einmalig für die Neural Engine übersetzt (~9 min auf M1). **Offen vor Production:** Push auch in der Release-App – Developer-ID-Profil mit Push neu erzeugen, dann `com.apple.developer.aps-environment = production` in `Earnote-iCloud.entitlements`; Schema nach Production übernehmen; Mac-Release. |
